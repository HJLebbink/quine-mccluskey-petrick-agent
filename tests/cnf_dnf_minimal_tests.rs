...
use qm_agent::cnf_dnf::{OptimizedFor, cnf_to_dnf, cnf_to_dnf_minimal};
        ...
        for opt in &OPT_MODES {
            ...
            let (dnf_minimal, dnf_regular) = if n_variables <= 16 {
                let minimal = match opt {
                    OptimizedFor::X64 => {
                        cnf_to_dnf_minimal::<Enc16>(&cnf, n_variables, OptimizedFor::X64)
                    }
                    OptimizedFor::Avx512_64bits => {
                        cnf_to_dnf_minimal::<Enc16>(&cnf, n_variables, OptimizedFor::Avx512_64bits)
                    }
                    OptimizedFor::Avx512_32bits => {
                        cnf_to_dnf_minimal::<Enc16>(&cnf, n_variables, OptimizedFor::Avx512_32bits)
                    }
                    OptimizedFor::Avx512_16bits => {
                        cnf_to_dnf_minimal::<Enc16>(&cnf, n_variables, OptimizedFor::Avx512_16bits)
                    }
                    OptimizedFor::Avx2_64bits => {
                        cnf_to_dnf_minimal::<Enc16>(&cnf, n_variables, OptimizedFor::Avx2_64bits)
                    }
                ...
                let regular = match opt {
                    ...
                    OptimizedFor::Avx512_64bits => {
                        cnf_to_dnf::<Enc16>(&cnf, n_variables, OptimizedFor::Avx512_64bits)
                    }
                    OptimizedFor::Avx512_32bits => {
                        cnf_to_dnf::<Enc16>(&cnf, n_variables, OptimizedFor::Avx512_32bits)
                    }
                    OptimizedFor::Avx512_16bits => {
                        cnf_to_dnf::<Enc16>(&cnf, n_variables, OptimizedFor::Avx512_16bits)
                    }
                    OptimizedFor::Avx2_64bits => {
                        cnf_to_dnf::<Enc16>(&cnf, n_variables, OptimizedFor::Avx2_64bits)
                    }
                    _ => panic!("Unsupported optimization mode"),
                };
                ...
                let minimal = match opt {
                    OptimizedFor::X64 => {
                        cnf_to_dnf_minimal::<Enc32>(&cnf, n_variables, OptimizedFor::X64)
                    }
                    OptimizedFor::Avx512_64bits => {
                        cnf_to_dnf_minimal::<Enc32>(&cnf, n_variables, OptimizedFor::Avx512_64bits)
                    }
                    OptimizedFor::Avx512_32bits => {
                        cnf_to_dnf_minimal::<Enc32>(&cnf, n_variables, OptimizedFor::Avx512_32bits)
                    }
                    OptimizedFor::Avx512_16bits => {
                        cnf_to_dnf_minimal::<Enc32>(&cnf, n_variables, OptimizedFor::Avx512_16bits)
                    }
                    OptimizedFor::Avx2_64bits => {
                        cnf_to_dnf_minimal::<Enc32>(&cnf, n_variables, OptimizedFor::Avx2_64bits)
                    }
                    _ => panic!("Unsupported optimization mode"),
                };
                let regular = match opt {
                    ...
                    OptimizedFor::Avx512_64bits => {
                        cnf_to_dnf::<Enc32>(&cnf, n_variables, OptimizedFor::Avx512_64bits)
                    }
                    OptimizedFor::Avx512_32bits => {
                        cnf_to_dnf::<Enc32>(&cnf, n_variables, OptimizedFor::Avx512_32bits)
                    }
                    OptimizedFor::Avx512_16bits => {
                        cnf_to_dnf::<Enc32>(&cnf, n_variables, OptimizedFor::Avx512_16bits)
                    }
                    OptimizedFor::Avx2_64bits => {
                        cnf_to_dnf::<Enc32>(&cnf, n_variables, OptimizedFor::Avx2_64bits)
                    }
                    _ => panic!("Unsupported optimization mode"),
                };
                (minimal.unwrap(), regular.unwrap())
                ...
                let minimal = match opt {
                    OptimizedFor::X64 => {
                        cnf_to_dnf_minimal::<Enc64>(&cnf, n_variables, OptimizedFor::X64)
                    }
                    OptimizedFor::Avx512_64bits => {
                        cnf_to_dnf_minimal::<Enc64>(&cnf, n_variables, OptimizedFor::Avx512_64bits)
                    }
                    OptimizedFor::Avx512_32bits => {
                        cnf_to_dnf_minimal::<Enc64>(&cnf, n_variables, OptimizedFor::Avx512_32bits)
                    }
                    OptimizedFor::Avx512_16bits => {
                        cnf_to_dnf_minimal::<Enc64>(&cnf, n_variables, OptimizedFor::Avx512_16bits)
                    }
                    OptimizedFor::Avx2_64bits => {
                        cnf_to_dnf_minimal::<Enc64>(&cnf, n_variables, OptimizedFor::Avx2_64bits)
                    }
                    _ => panic!("Unsupported optimization mode"),
                };
                let regular = match opt {
                    ...
                    OptimizedFor::Avx512_64bits => {
                        cnf_to_dnf::<Enc64>(&cnf, n_variables, OptimizedFor::Avx512_64bits)
                    }
                    OptimizedFor::Avx512_32bits => {
                        cnf_to_dnf::<Enc64>(&cnf, n_variables, OptimizedFor::Avx512_32bits)
                    }
                    OptimizedFor::Avx512_16bits => {
                        cnf_to_dnf::<Enc64>(&cnf, n_variables, OptimizedFor::Avx512_16bits)
                    }
                    OptimizedFor::Avx2_64bits => {
                        cnf_to_dnf::<Enc64>(&cnf, n_variables, OptimizedFor::Avx2_64bits)
                    }
                    _ => panic!("Unsupported optimization mode"),
                };
                (minimal.unwrap(), regular.unwrap())
            ...
            assert!(
                dnf_minimal.len() <= dnf_regular.len(),
            ...
            if !dnf_regular.is_empty() {
                let min_size = dnf_regular.iter().map(|x| x.count_ones()).min().unwrap();
...
fn test_cnf_to_dnf_minimal_edge_cases() {
    ...
    let cnf: Vec<u64> = vec![];
    let dnf = cnf_to_dnf_minimal::<qm_agent::qm::Enc16>(&cnf, 4, OptimizedFor::X64).unwrap();
    ...
    let dnf_single =
        cnf_to_dnf_minimal::<qm_agent::qm::Enc16>(&cnf_single, 4, OptimizedFor::X64).unwrap();
    assert_eq!(
        dnf_single.len(),
        2,
        "Single clause with 2 literals should produce 2 terms"
    );
    ...
    for &term in &dnf_single {
        assert_eq!(
            term.count_ones(),
            1,
            "Each term should be a single literal, but got {:b}",
            term
        );
    }
    ...
    let dnf_one_bit =
        cnf_to_dnf_minimal::<qm_agent::qm::Enc16>(&cnf_one_bit, 4, OptimizedFor::X64).unwrap();
    assert_eq!(
        dnf_one_bit.len(),
        1,
        "Single literal clause should produce 1 term"
    );
