...
pub enum CnfDnfError {
    ...
    EncodingCapacityExceeded { n_bits: usize, max_vars: usize },
    ...
    TooManyVariables { n_variables: usize },
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            CnfDnfError::EncodingCapacityExceeded { n_bits, max_vars } => {
                write!(
                    f,
                    "n_bits ({}) exceeds encoding maximum ({})",
                    n_bits, max_vars
                )
            }
            CnfDnfError::OptimizationLevelExceeded {
                n_bits,
                optimization,
                max_bits,
            } => {
                write!(
                    f,
                    "n_bits ({}) exceeds {} maximum ({} bits)",
                    n_bits, optimization, max_bits
                )
            }
            CnfDnfError::TooManyVariables { n_variables } => {
                write!(
                    f,
                    "too many different variables; found {} variables",
                    n_variables
                )
            }
        }
